...
namespace BeerTap.WebApi.Infrastructure.Installers
{
    ...
    public class DomainServicesInstaller : IWindsorInstaller
    {
        ...
        void RegisterUsingContainer(IWindsorContainer container)
        {
            ...
            container
                .Register(Classes.FromAssembly(_apiDomainServicesAssembly).BasedOn<IBeerTapService>()
                    .Configure(
                        cr =>
                        {
                            var x =
                                cr.Interceptors(InterceptorReference.ForKey(LoggingConstants.DomainServiceLogger))
                                    .Anywhere;
                        }
                    )
                    .WithServiceFromInterface());
            ...
            get
            {
                return
                    _apiDomainServiceInterfacesAssembly
                        ...
                        .Where(t => t.IsInterface && typeof(IBeerTapService).IsAssignableFrom(t));
            }
        }
    }
}
